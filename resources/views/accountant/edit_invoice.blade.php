            ...
            <div class="card">
              ...
              <div class="tab-content p-4" id="pills-tabContent-basic-forms">
                <div class="tab-pane tab-example-design fade show active" id="pills-basic-forms-design" role="tabpanel"
                  ...
                  <form method="POST" action="{{ route('accountant.update.invoice', Crypt::encrypt($invoice->id)) }}">
                    @csrf
                    <!-- Input -->
                    <div class="mb-3">
                      <label class="form-label" for="textInput">Invoice Number</label>
                      <input type="text" id="invoiceSerial" name="invoiceSerial"
                        class="form-control @error('invoiceSerial') is-invalid @enderror"
                        value="{{$invoice->invoiceSerial}}" readonly>
                      @error('invoiceSerial')
                    ...
                    <div class="mb-3">
                      <label class="form-label" for="selectOne">Product (Client)</label>
                      <select id="projectId" name="projectId"
                        class="form-select @error('projectId') is-invalid @enderror">
                        <option value="" {{old('projectId')=='' ? 'selected' : '' }}>------</option>
                        @foreach ($projects as $item)
                        <option value="{{$item->id}}" {{$invoice->project->id || old('projectId') == $item->id ?
                          'selected' : ''}}>
                    ...
                    <div class="mb-3">
                      ...
                      <input type="number" id="amountToPay" name="amountToPay"
                        class="form-control @error('amountToPay') is-invalid @enderror"
                        value="{{$invoice->amountToPay}}" required>
                      @error('amountToPay')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                      </span>
                    ...
                    <div class="mb-3">
                      ...
                      <input type="date" id="dueDate" name="dueDate"
                        ...
                        value="{{old('dueDate') ? old('dueDate'): Carbon\Carbon::parse($invoice->dueDate)->format('Y-m-d')}}"
                        required>
                      @error('dueDate')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
